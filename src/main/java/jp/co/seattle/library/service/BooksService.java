...
public class BooksService {
	...
	public List<BookInfo> getBookList() {
		...
		List<BookInfo> getedBookList = jdbcTemplate.query(
				"SELECT id,title,author,publisher,publish_date,isbn,description,thumbnail_url FROM books ORDER BY title ASC;",
	...
	public void updateBook(BookDetailsInfo bookInfo) {
		...
		if (bookInfo.getThumbnailUrl() == null) {
			...
			// TODO 取得した書籍情報を更新するようにSQLを修正（タスク５）
			sql = "UPDATE books SET title = ?, author = ?, publisher = ?, publish_date = ?, thumbnail_name = ?, thumbnail_date = ?, isbn = ?, description = ?, id = ?, upd_date = CURRENT_DATE  WHERE id ;";
			jdbcTemplate.update(sql, bookInfo.getTitle(), bookInfo.getAuthor(), bookInfo.getPublisher(),
